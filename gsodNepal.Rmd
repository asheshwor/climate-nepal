...
dhm.monsoon <- read.csv(dhm.file, header = TRUE, colClasses = c("numeric", "character",
                                         "character", "numeric",
                                         "numeric", "numeric"),
...
p.dhm <- qplot(as.numeric(year),
               ...
               size=I(1),
               alpha=I(.9),
               main= expression(atop("Official monsoon onset day - Nepal (1968-2018)",
                                     "Source: DHM")),
               xlab="Year", 
               ylab="Monsoon onset day")
p.dhm <- p.dhm + stat_smooth(method="lm")
p.dhm <- p.dhm + geom_point()
p.dhm <- p.dhm +
